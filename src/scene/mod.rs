...
use glium::{framebuffer::SimpleFrameBuffer, glutin::surface::WindowSurface, Display, Texture2d};
use light::Light;
use objects::{physics::{physics_object_factory, PhysicsObject}, renderable::renderobjects::RenderObject, WorldObject};
use rand::Rng;
use winit::window::Window;

use crate::{rendering::{render::{self, Renderer}, render_camera::RenderCamera}, util::{input_handler::{self, InputHandler}, load_texture}};
...
impl<'a> Scene<'a>{
    pub fn new(camera: RenderCamera, lights: Option<Vec<Light>>) -> Scene<'a>{
        Scene { world_objects: Vec::new(), lights: lights.unwrap_or(Vec::new()), renderers: HashMap::new(), textures: HashMap::new(),camera: camera }
    }
    ...
    pub fn draw(&mut self, fbo: &mut SimpleFrameBuffer<'_>){
        for render in &mut self.world_objects{
            ...
            if render_id.is_some(){
                render.draw(fbo, &self.camera, self.renderers.get(render_id.as_ref().unwrap().as_str()).unwrap(), &mut self.textures.get(&render.id));
            }
    ...
    pub fn init_gravity_scene(window: &Window, display: &Display<WindowSurface>) -> Scene<'a>{
        ...
        let mut return_scene = Scene { world_objects: solar_system, lights: Vec::new(), renderers: render_map,camera: RenderCamera::new(Vec3::new(0.0, 0.0, 15.0), Vec3::new(0.0, 0.0, 0.0), Vec3::new(0.0, 1.0, 0.0), Vec3::new(0.0, 0.0, -1.0), window.inner_size().into()), textures: HashMap::new() };
