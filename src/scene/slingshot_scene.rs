use crate::{managers::{CreatorManager, ObjectManager, RenderManager}, scene::{Scene, SceneContent, objects::{self, SceneObjectBehaviour, SplineObject, object_creator::{create_generalized_grid, paraboloid_grid}, physics::{bodies::RigidBody, controllers::{gravity_controller::PlayerGravity, line_controller::LineController, mouse_movement_controller::MouseDragController, movement_controller::SimpleMover, prediction_controller::PredictionController}}, renderable::{MeshRenderer, SplineRenderer, TextureMeshRenderer}, transform::Transform}}};
...
impl Scene{
    pub fn init_slingshot_scene(window: &Window, display: &Display<WindowSurface>, size: (u32,u32)) -> Scene{
        ...
        let planet_renderer  = Renderer::init(&mut render_manager, display, include_bytes!(r"../../shaders/planet/vert.glsl"), include_bytes!(r"../../shaders/planet/frag.glsl"), include_bytes!(r"../../objects/Suzanne.obj"), None).unwrap();
        ...
        let player = objects.add(|id| {
            let mut player = WorldObject::new(id, Transform::new(), None, PhysicsObject::RigidBody(RigidBody::new(1.0)), None);
            player.data.transform.translate(Vec3::new(0.0, 0.0, 0.0));
            player.data.transform.scale(Vec3::new(0.4, 0.4, 0.4));
            SceneObject::World(player)
        }, true);
        ...
        let planet1 = objects.add(|id| {
            let mut planet = WorldObject::new(id,Transform::new(), Some(Box::new(TextureMeshRenderer::new("planet".to_string(), planet_texture.clone()))), PhysicsObject::RigidBody(RigidBody::new(12.0)), None);
            planet.data.transform.translate(Vec3::new(3.0, 2.0, 0.0));
            SceneObject::World(planet)
        }, false);


        let planet2 = objects.add(|id| {
            let mut planet = WorldObject::new(id,Transform::new(), Some(Box::new(TextureMeshRenderer::new("planet".to_string(), planet_texture.clone()))), PhysicsObject::RigidBody(RigidBody::new(12.0)), None);
            planet.data.transform.translate(Vec3::new(-3.0, -2.0, 0.0));
            SceneObject::World(planet)
        }, false);
        ...
        camera.set_following(&player);


        let spline = objects.add(|id| {
        ...
        for _ in 0..100{
            let build = |id| {
                let mut add_planet = WorldObject::new(id, Transform::new(),Some(Box::new(MeshRenderer::new("planet".to_string()))), PhysicsObject::RigidBody(RigidBody::new(rng.random_range(0.1..20.0))), None);
                ...
                SceneObject::World(add_planet)
            };
            objects.add(build, false);
        }
        let (world_texture, depth_world_texture) = create_render_textures(&display,size.0, size.1);



        let mut return_scene= Scene { content: SceneContent { time: 0.0, objects, camera, lights: Vec::new(), render_objects: Vec::new()}, render_manager, controllers: vec![/*Box::new(mouse), Box::new(line), */Box::new(gravity), Box::new(prediction), Box::new(movement), Box::new(player_movement)], scene_tex: world_texture, scene_depth: depth_world_texture};
