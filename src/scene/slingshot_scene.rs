use crate::{assetmanager::RenderManager, scene::{Scene, SceneContent, objects::{self, physics::{bodies::RigidBody, controllers::mouse_movement_controller::MouseDragController}, renderable::MeshRenderer, transform::Transform}}, util::ray_library::{mouse_ray, ndc_to_intersection, ray_plane_intersection}};
use core::f32;
...
impl Scene{
    pub fn init_slingshot_scene(window: &Window, display: &Display<WindowSurface>, size: (u32,u32)) -> Scene{
        ...
        let things = vec![player, planet1, worldmouse];

        let (world_texture, depth_world_texture) = create_render_textures(&display,size.0, size.1);



        let mut return_scene= Scene { content: SceneContent { time: 0.0, world_objects: things, camera, lights: Vec::new()}, render_manager, controllers: vec![Box::new(mouse)], textures: HashMap::new(), scene_tex: world_texture, scene_depth: depth_world_texture};
